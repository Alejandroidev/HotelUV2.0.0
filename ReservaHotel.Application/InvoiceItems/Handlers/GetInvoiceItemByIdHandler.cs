...
{
    /// <summary>
    /// Handles requests to retrieve an invoice item by identifier.
    /// </summary>
    /// <remarks>
    /// Example:
    /// var result = await _mediator.Send(new GetInvoiceItemByIdQuery(id), ct);
    /// </remarks>
    public class GetInvoiceItemByIdHandler : IRequestHandler<GetInvoiceItemByIdQuery, CustomWebResponse>
    {
        private readonly IReadRepository<InvoiceItem> _repo;
        private readonly IMapper _mapper;

        public GetInvoiceItemByIdHandler(IReadRepository<InvoiceItem> repo, IMapper mapper)
        {
            _repo = repo;
            _mapper = mapper;
        }
        ...
        /// <inheritdoc />
        public async Task<CustomWebResponse> Handle(GetInvoiceItemByIdQuery request, CancellationToken ct)
        {
